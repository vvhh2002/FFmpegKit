...
enum Library: String, CaseIterable {
    ...
    var isFFmpegDependentLibrary: Bool {
        switch self {
        case .vulkan, .libshaderc, .libglslang, .lcms2, .libplacebo, .libdav1d, .gmp, .gnutls, .libsrt, .libzvbi, .libfontconfig, .libbluray, .openssl:
            return true
        case .libsmbclient:
...
//            "PKG_CONFIG_PATH": pkgConfigPath,
    ...
    private func createFramework(framework: String, platform: PlatformType) throws -> String? {
        ...
        // macOS requires versioned framework bundles (Versions/A/) instead of shallow bundles.
        // Reorganize the flat bundle into the standard structure with symlinks.
        if platform == .macos || platform == .maccatalyst {
            let versionsA = frameworkDir + "Versions/A"
            try FileManager.default.createDirectory(at: versionsA, withIntermediateDirectories: true, attributes: nil)
            // Move Headers, Modules, and the binary into Versions/A/
            try FileManager.default.moveItem(at: frameworkDir + "Headers", to: versionsA + "Headers")
            try FileManager.default.moveItem(at: frameworkDir + "Modules", to: versionsA + "Modules")
            try FileManager.default.moveItem(at: frameworkDir + framework, to: versionsA + framework)
            // Move Info.plist into Versions/A/Resources/
